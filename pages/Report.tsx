...
import { Download, Filter, Search, Loader2, Calendar, FileText, CheckCircle2, AlertTriangle } from 'lucide-react';
...
export const Report: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-24 md:pb-0 animate-fade-in">
      ...
      {/* List Container */}
      <div className="bg-transparent md:bg-white md:dark:bg-gray-800 md:rounded-2xl md:shadow-sm md:border md:border-gray-100 md:dark:border-gray-700 overflow-hidden animate-slide-up">
        ...
        <div className="md:hidden space-y-4">
           {filteredLogs.map(log => (
              <div key={log.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                 <div className="flex justify-between items-start mb-3">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <span className="text-xs text-gray-400 font-mono">{new Date(log.timestamp).toLocaleDateString('th-TH')} {new Date(log.timestamp).toLocaleTimeString('th-TH', {hour: '2-digit', minute:'2-digit'})}</span>
                        </div>
                        <h3 className="font-bold text-gray-800 dark:text-white">{log.productName}</h3>
                        <span className="text-xs text-gray-400 bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded font-mono">{log.barcode}</span>
                    </div>
                    <span className={`px-2 py-1 rounded-full text-xs font-bold border flex items-center gap-1 ${log.status === 'Pass' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
                       {log.status === 'Pass' ? <CheckCircle2 size={12} /> : <AlertTriangle size={12} />}
                       {log.status}
                    </span>
                 </div>
                 ...
                 <div className="flex justify-between items-end border-t border-gray-100 dark:border-gray-700 pt-3">
                     <div className="text-sm">
                         <p className="text-gray-500 dark:text-gray-400 text-xs uppercase">ราคาขาย</p>
                         <p className="font-bold text-gray-800 dark:text-white">฿{log.sellingPrice.toFixed(2)}</p>
                     </div>
                     {log.reason && (
                        <div className="text-right max-w-[60%]">
                            <p className="text-gray-500 dark:text-gray-400 text-xs uppercase">หมายเหตุ</p>
                            <p className="text-sm text-gray-600 dark:text-gray-300 truncate">{log.reason}</p>
                        </div>
                     )}
                 </div>
              </div>
           ...
           {filteredLogs.length === 0 && (
             <div className="p-10 text-center text-gray-400 bg-white dark:bg-gray-800 rounded-xl">
